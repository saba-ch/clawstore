...
import { agents, versions, agentTags, versionAssets, profiles, categories } from "../../db/schema";
...
app.post("/publish", async (c) => {
  try {
    ...
    const [categoryRow] = await db
      .select()
      .from(categories)
      .where(eq(categories.id, manifest.category))
      .limit(1);

    if (!categoryRow) {
      throw new AppError(
        "invalid_category",
        `Category '${manifest.category}' does not exist`,
        400
      );
    }
    ...
    throw new AppError("publish_failed", message, 500);
  }
});
